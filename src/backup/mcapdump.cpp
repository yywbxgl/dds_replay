...
int main(int argc, char* argv[]) {
    ...
    for (const auto& msgView : messages) {
      ...
      } else if (channel.topic == "/ddsrecorder/status") {
        DdsRecorderStatus status;
        statusType.deserialize(&payload, &status);
        printf("get status. log_time=%lu, previous=%s  current=%s   info=%s\n", 
          msgView.message.logTime,  status.previous().c_str(), status.current().c_str(), status.info().c_str());
      }

      payload.data = nullptr;  // ps.
      // std::cout << "[" << channel.topic << "] " << ToString(msgView.message) << "\n";
    }
